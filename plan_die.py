...
def find_die_plan(range_start, range_end, step_list):
    assert range_start < range_end

    for sobj in step_list:
        chara_step, _ = sobj

        step = get_step(0, chara_step)
        if step < range_start:
            nxt = find_die_plan(range_start - step, range_end - step, step_list)
            if nxt is None:
                continue
            else:
                return [sobj] + nxt
